...
}
{
    ...
    {
        return new ru.ibs.dtm.query.execution.plugin.adqm.calcite.eddl.SqlCreateDownloadExternalTable(s.end(this),
...
}
{
    <DOWNLOAD> <EXTERNAL> <TABLE> ifExists = IfExistsOpt() id = CompoundIdentifier() {
        return new ru.ibs.dtm.query.execution.plugin.adqm.calcite.eddl.SqlDropDownloadExternalTable(s.end(this), ifExists, id);
    }
}
